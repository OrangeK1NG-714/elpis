<template>
    <el-row type="flex" align="middle" class="form-item">
        ...
        <el-row class="item-value">
            <el-input v-model="dtoValue" v-bind="schema.option" class="component"
                :class="validTips ? 'valid-border' : ''" :placeholder="placeholder" @focus="onFocus"
    ...
    if (dtoValue.value) {
        ...
        if (!valid && validate.errors && validate.errors[0]) {
            ...
            if (keyword === 'type') {
                ...
                validTips.value = `格式错误，${params.pattern}`
            } else {
